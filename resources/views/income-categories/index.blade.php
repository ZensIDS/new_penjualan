...
@section('page-title', 'Kategori Pemasukan')

@section('content')
<div
    x-data="incomeCategoryPage()"
    x-cloak
>
    <div class="flex items-center justify-between mb-6">
        <div>
            <p class="text-2xl font-display font-semibold tracking-tight">{{ $incomeCategories->total() }}</p>
            <p class="text-sm text-ink/50">kategori pemasukan</p>
        </div>
    ...
    @include('partials.date-range-filter', ['routeName' => 'income-categories.index', 'dateLabel' => 'Tgl Dibuat'])
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-ink/[0.06]">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-10 text-center text-ink/40">Belum ada kategori pemasukan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        >
            ...
            <div class="p-6">
                <div class="flex items-center gap-3 mb-5">
                    ...
                    <h2 class="font-display font-semibold text-lg" x-text="editing ? 'Edit Kategori Pemasukan' : 'Tambah Kategori Pemasukan'"></h2>
                </div>

                <form @submit.prevent="submit()">
                    <div>
                        <label class="block text-sm font-medium mb-1.5">Nama <span class="text-red-600">*</span></label>
                        <input type="text" x-model="form.name" placeholder="Modal Disetor, Pinjaman, Sewa, dll"
        ...
        return {
            ...
            async submit() {
                ...
                const url = this.editing
                    ? `{{ url('income-categories') }}/${this.editing.id}`
                    : `{{ route('income-categories.store') }}`;
            ...
            async remove(id) {
                ...
                const { ok, data } = await window.ajaxSend(`{{ url('income-categories') }}/${id}`, 'DELETE');
