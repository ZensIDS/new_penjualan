...
>
    <div class="flex items-center justify-between mb-6">
        <div>
            <p class="text-2xl font-display font-semibold tracking-tight">{{ $categories->total() }}</p>
            <p class="text-sm text-ink/50">kategori produk</p>
        </div>
        ...
        >
            ...
            <div class="p-6">
                <div class="flex items-center gap-3 mb-5">
                    ...
                    <h2 class="font-display font-semibold text-lg" x-text="editing ? 'Edit Kategori' : 'Tambah Kategori'"></h2>
                </div>
                ...
                <form @submit.prevent="submit()">
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium mb-1.5">Slug <span class="text-ink/40 font-normal">(opsional, otomatis dari nama)</span></label>
                            <input type="text" x-model="form.slug"
                                   class="w-full rounded-xl border border-ink/12 px-3.5 py-2.5 text-sm focus:outline-none focus:border-amber-500 focus:ring-4 focus:ring-amber-500/15 transition-shadow">
                            <p class="text-xs text-red-600 mt-1" x-text="errors.slug?.[0]"></p>
                        </div>
    ...
    function categoryPage() {
        return {
            ...
            openEdit(category) {
                ...
                this.form = { name: category.name, slug: category.slug, description: category.description };
                this.errors = {};
                this.modalOpen = true;
            },
            ...
            async submit() {
                ...
                const url = this.editing
                    ? `{{ url('categories') }}/${this.editing.id}`
            ...
            async remove(id) {
                ...
                const { ok, data } = await window.ajaxSend(`{{ url('categories') }}/${id}`, 'DELETE');
