...
>
    ...
    <form method="POST" action="{{ route('purchase-orders.store') }}">
                ...
                <template x-for="(item, index) in items" :key="item.key">
                    <div class="p-5 grid grid-cols-1 @4xl:grid-cols-12 gap-3 sm:items-start">
                        <div class="@4xl:col-span-5">
                            <label class="block text-xs font-medium text-ink/50 mb-1.5">Produk</label>
                            <div class="relative">
                        ...
                        <div class="@4xl:col-span-2">
                            <label class="block text-xs font-medium text-ink/50 mb-1.5">Qty</label>
                        ...
                        <div class="@4xl:col-span-3">
                            <label class="block text-xs font-medium text-ink/50 mb-1.5">Harga Beli / Unit</label>
                            <div class="relative">
                        ...
                        <div class="@4xl:col-span-1">
                            <label class="block text-xs font-medium text-ink/50 mb-1.5">Subtotal</label>
        ...
        <div class="rounded-2xl border border-ink/10 bg-white shadow-card overflow-hidden mb-6">
            ...
            <p class="px-6 py-8 text-sm text-ink/40 text-center" x-show="extraCosts.length === 0">Belum ada biaya tambahan.</p>
                        ...
                        <div class="@4xl:col-span-3">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" inputmode="numeric"
                                       ...
                                       class="w-full rounded-xl border border-ink/12 pl-9 pr-3.5 py-2.5 text-sm tnum focus:outline-none focus:border-amber-500 focus:ring-4 focus:ring-amber-500/15 transition-shadow">
                                <input type="hidden" :name="'extra_costs['+index+'][amount]'" :value="cost.amount">
                            </div>
                        </div>

                        <div class="@4xl:col-span-4">
                            <label class="block text-xs font-medium text-ink/50 mb-1.5">Keterangan</label>
                            <input type="text" :name="'extra_costs['+index+'][description]'" x-model="cost.description" placeholder="Opsional"
                                   class="w-full rounded-xl border border-ink/12 px-3.5 py-2.5 text-sm focus:outline-none focus:border-amber-500 focus:ring-4 focus:ring-amber-500/15 transition-shadow">
                        </div>

                        <div class="@4xl:col-span-1 flex sm:justify-end sm:pt-6">
                            <button type="button" @click="removeExtraCost(cost.key)"
                                    class="text-red-600/70 hover:text-red-700 p-1.5" title="Hapus biaya">
                                <svg viewBox="0 0 24 24" class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M6 6l12 12M18 6L6 18"/></svg>
                            </button>
                        </div>
                    </div>
            ...
            <div class="px-6 py-4 bg-ink/[0.02] flex items-center justify-between" x-show="extraCosts.length > 0">
                <span class="text-sm font-medium text-ink/60">Total Biaya Tambahan</span>
                <span class="font-display font-semibold text-lg tnum" x-text="'Rp ' + formatRupiah(totalExtraCosts)"></span>
            </div>
        </div>
        @error('extra_costs')<p class="text-xs text-red-600 -mt-4 mb-6">{{ $message }}</p>@enderror
...
<script>
    function poCreateForm(suppliers, products, initialItems) {
        return {
            ...
            removeExtraCost(key) {
                this.extraCosts = this.extraCosts.filter(c => c.key !== key);
            },

            get totalExtraCosts() {
                return this.extraCosts.reduce((sum, c) => sum + (parseFloat(c.amount) || 0), 0);
            },
